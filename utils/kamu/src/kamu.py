...
def fetchImagesFromSubPage(subPageLink, chapterIdx, chapterDir):
    ...
    for img in imgs:
        imgIdx = imgIdx + 1
        imgLink = img['data-original'].strip()
        imgExt = imgLink.split('.')[-1]
        targetImgPath = chapterDir + ("%03d.%s" % (imgIdx, imgExt))
        ret = downloadImg(imgLink, targetImgPath)
        if ret:
...
def getComicNameFromSoup(soup):
    div = soup.find('div', attrs = {"class": "cy_title"})
    if div == None:
        logger.info("Fail to get <div class='cy_title'> from soup")
        return ''
    h1 = div.find('h1')
    if h1 == None:
        logger.info("Fail to get <div class='cy_title'>.<h1> from soup")
        return ''
    return h1.string


def getChapterLinks(mainPageSoup):
    liList = mainPageSoup.find_all('li', attrs = {'class': 'chapter__item'})
    finalChapterLink = ''
    maxChapterIdx = -1
    chapterLinks = {}
    for li in liList:
        a = li.find('a')
        if a == None:
            logger.info("No <a> under <ls>" % li.prettify())
            continue
        chapterIdx = -1
        for content in a.contents:
            nums = re.findall('第\s*(\d+)', str(content))
            if len(nums) > 0:
...
def getValidComicInfoContent(comicInfoFilePath):
    ...
    if 'links' not in comicInfo.keys() or 'kamu' not in comicInfo['links'].keys():
        logger.info("No <links> or <links/kamu> in [%s] " % comicInfoFilePath)
        return None
    link = comicInfo['links']['kamu']
    if link == '':
        logger.info("No valid kamu link in [%s] " % comicInfoFilePath)
        return None
    return comicInfo
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        for comicDir in comicDirList:
            ...
            if 'kamu' not in comicInfo['links'].keys():
                continue
            link = comicInfo['links']['kamu']
